...
```{r}
sep_old <- sep_old %>% 
  rename(ugradold = u_grad)

sep_new <- sep_new %>% 
  rename(ugradnew = u_grad)

sep_diff <- inner_join(sep_new, sep_old, by = c("id", "title", "name", "department"))

sep_diffa <- inner_join(sep_new, sep_old, by = c("id", "title", "name", "department"))%>%
  mutate(diff = (ugradnew - ugradold))%>%
  arrange(desc(diff))%>%
  slice(1:5)

sep_diffb <- inner_join(sep_new, sep_old, by = c("id", "title", "name", "department"))%>%
  mutate(diff = (ugradnew - ugradold))%>%
  arrange(diff)%>%
  slice(1:5)
...
sep_diffa %>%
  gt()%>%
  tab_header(title = "Five Harvard Courses with Biggest Increase")
...
sep_diffb %>%
  gt()%>%
  tab_header(title = "Five Harvard Courses with Biggest Decrease")%>%
  cols_label(c("id" = "Course ID", 
               "title" = "Title"))
```
